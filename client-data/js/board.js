...
function handleMarker(evt) {
  ...
  if (
    Tools.showMyPointer &&
    !Tools.suppressPointerMsg &&
    lastPointerUpdate < cur_time - 1000 / MAX_CURSOR_UPDATES_PER_SECOND
  ) {
...
setInterval(function () {
  for (var i in cursors) {
    if (
      Date.now() - cursorLastUse[cursors[i].id] > 3000 &&
      cursors[i].style.opacity != 0.2
    ) {
      $(cursors[i]).fadeTo(1500, 0.2);
    }
  }
}, 2000);
...
function movePointer(message) {
  ...
  if (!cursor) {
    ...
    Tools.svg.getElementById("cursors").innerHTML =
      "<circle class='opcursor' id='cursor" +
      message.socket +
      "' cx='0' cy='0' r='10' fill='orange' />";
  ...
  if (message.c) cursor.setAttributeNS(null, "fill", message.c);
...
function updateActivityMonitor(cursor) {
  ...
  if (bounding.left < -20) {
    ...
    bounding.right >
    20 + (window.innerWidth || document.documentElement.clientWidth)
  ) {
    region[3] = 0;
    region[1] = 1;
  }
    ...
    return tmp.add(function (elem) {
      ...
      if (menu) {
        ...
        (function () {
          ...
          Tools.menus[toolName].show = function (show) {
            if (!show) {
              ...
              Tools.menus[toolName].menu.style.transform =
                "translate3d(50px,  " +
                (-scrollTop + Tools.menus[toolName].y) +
                "px, 0px)";
          ...
          Tools.menus[toolName].menuOpen = function () {
            return !hidden;
          };
          ...
          var handleScroll = function () {
            Tools.menus[toolName].menu.style.transform =
              "translate3d(50px,  " +
              (-this.scrollTop + Tools.menus[toolName].y) +
              "px, 0px)";
          };
          ...
          var listen = function (e) {
            ...
            if (e.target.id != elem.id && !$(elem).find(e.target).length)
              onButton = false;
            if (
              !(e.target.id && e.target.id == "popover-" + toolName) &&
              !$("#popover-" + toolName).find(e.target).length
            )
              onMenu = false;
...
Tools.send = function (data, toolName) {
  ...
  if (message.data.type != "c" && message.data.type != "e") {
    ...
    if (
      ...
      Tools.msgs.length &&
      Tools.msgs[Tools.msgs.length - 1].type == "update"
    ) {
      if (
        (message.data.gid &&
          Tools.msgs[Tools.msgs.length - 1].gid == message.data.gid) ||
        (message.data.id &&
          !Array.isArray(message.data.id) &&
...
(function () {
  ...
  window.addEventListener("scroll", function onScroll() {
    ...
    scrollTimeout = setTimeout(function updateHistory() {
      var hash =
        "#" + (x | 0) + "," + (y | 0) + "," + Tools.getScale().toFixed(2);
...
Tools.applyShortcuts = function (shortcuts, toolName) {
  if (!shortcutsInit) {
    ...
    window.addEventListener("keydown", function (e) {
      for (var i = 0; i < Tools.shortcuts.changeToolList.length; i++) {
        if (
          e.key === Tools.shortcuts.changeToolList[i].key &&
  ...
  if (shortcuts.changeTool)
    Tools.shortcuts.changeToolList.push({
      toolName: toolName,
      key: shortcuts.changeTool,
    });
...
Tools.adjustBox = function (el, r, m) {
  ...
  var xstroke =
    Math.hypot(
      Math.cos(rot) * result.scale[0],
      Math.sin(rot) * result.scale[1]
    ) *
    strokeWidth *
    0.6;
  var ystroke =
    Math.hypot(
      Math.cos(rot) * result.scale[1],
      Math.sin(rot) * result.scale[0]
    ) *
    strokeWidth *
    0.6;
  r.x -= xstroke;
...
function resize_view() {
  ...
  svgWidth = Tools.svg.width.baseVal.value = Math.max(
    screenWidth + 2000,
    screenWidth * 2.5
  );
  svgHeight = Tools.svg.height.baseVal.value = Math.max(
    screenHeight + 2000,
    screenHeight * 2.5
  );
  Tools.svg.setAttributeNS(
    null,
    "viewBox",
